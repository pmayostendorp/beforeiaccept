...
d.over=b(d.over);
return this._scrollable().each(function(){var o=this,m=c(o),n=f,k,l={},h=m.is("html,body");
switch(typeof n){case"number":case"string":if(/^([+-]=)?\d+(\.\d+)?(px|%)?$/.test(n)){n=b(n);
break
}n=c(n,this);
case"object":if(n.is||n.style){k=(n=c(n)).offset()
}}c.each(d.axis.split(""),function(q,r){var u=r=="x"?"Left":"Top",s=u.toLowerCase(),w="scroll"+u,p=o[w],g=a.max(o,r);
if(k){l[w]=k[s]+(h?0:p-m.offset()[s]);
if(d.margin){l[w]-=parseInt(n.css("margin"+u))||0;
l[w]-=parseInt(n.css("border"+u+"Width"))||0
}l[w]+=d.offset[s]||0;
if(d.over[s]){l[w]+=n[r=="x"?"width":"height"]()*d.over[s]
}}else{var v=n[s];
l[w]=v.slice&&v.slice(-1)=="%"?parseFloat(v)/100*g:v
}if(/^\d+$/.test(l[w])){l[w]=l[w]<=0?0:Math.min(l[w],g)
}if(!q&&d.queue){if(p!=l[w]){j(d.onAfterFirst)
}delete l[w]
}});
j(d.onAfter);
function j(g){m.animate(l,e,d.easing,g&&function(){g.call(this,f,d)
})
}}).end()
};
...
PDC.ReviewsRatings={cache:{},ratingsQueue:{},feedbackQueue:{},getRating:function(a,d,c){if(PDC.ReviewsRatings.ratingsQueue[d]){pdclog("reviews-ratings/get-rating: queued for "+d);
PDC.ReviewsRatings.ratingsQueue[d].push(c);
return
}if(PDC.ReviewsRatings.cache[d]&&PDC.ReviewsRatings.cache[d].rating){pdclog("reviews-ratings/get-rating: cached for "+d);
c(PDC.ReviewsRatings.cache[d].rating);
return
}pdclog("reviews-ratings/get-rating: calling for "+d);
if(!PDC.ReviewsRatings.cache[d]){PDC.ReviewsRatings.cache[d]={}
}PDC.ReviewsRatings.ratingsQueue[d]=[];
var b={gameId:d};
if($.cookie("userinfo")){b.userInfoCookie=$.cookie("userinfo")
}b.action="getaverageRating";
if(PDC.Region.getRegion()=="SCEA"){$.ajax({url:a,type:"GET",dataType:"json",data:b,success:function(f){pdclog("reviews-ratings/get-rating: success for "+d);
PDC.ReviewsRatings.cache[d].rating=f;
c(f);
if(PDC.ReviewsRatings.ratingsQueue[d]){pdclog("reviews-ratings: get-rating: queue="+PDC.ReviewsRatings.ratingsQueue[d].length);
for(var e=0;
e<PDC.ReviewsRatings.ratingsQueue[d].length;
e++){PDC.ReviewsRatings.ratingsQueue[d][e](f)
}}PDC.ReviewsRatings.ratingsQueue[d]=null
},error:function(f,e){pdclog("ratings-review/get-rating error: "+e)
}})
}},getFeedback:function(b,g,e,c,a,f){if(a){pdclog("reviews-ratings/get-feedback: cache mode for "+g);
if(PDC.ReviewsRatings.feedbackQueue[g] instanceof Array){pdclog("reviews-ratings/get-feedback: queued for "+g);
PDC.ReviewsRatings.feedbackQueue[g].push(f);
pdclog("feedback-queue inner");
pdclog(PDC.ReviewsRatings.feedbackQueue);
return
}if(PDC.ReviewsRatings.cache[g]&&PDC.ReviewsRatings.cache[g].feedback){pdclog("reviews-ratings/get-feedback: cached for "+g);
f(PDC.ReviewsRatings.cache[g].feedback);
return
}if(!(PDC.ReviewsRatings.feedbackQueue[g] instanceof Array)){PDC.ReviewsRatings.feedbackQueue[g]=[]
}}pdclog("feedback-queue outer");
pdclog(PDC.ReviewsRatings.feedbackQueue);
pdclog("reviews-ratings/get-feedback: calling for "+g);
if(!PDC.ReviewsRatings.cache[g]){PDC.ReviewsRatings.cache[g]={}
}var d={gameId:g,pageNum:e!=null&&e>-1?e:1,sortOrder:c};
if($.cookie("userinfo")){d.userInfoCookie=$.cookie("userinfo")
}d.action="GetGamefeedBack";
if(PDC.Region.getRegion()=="SCEA"){$.ajax({url:b,type:"GET",dataType:"json",data:d,success:function(j){pdclog("reviews-ratings/get-feedback: success for "+g);
PDC.ReviewsRatings.cache[g].feedback=j;
f(j);
if(PDC.ReviewsRatings.feedbackQueue[g]){pdclog("reviews-ratings: get-feedback: queue="+PDC.ReviewsRatings.feedbackQueue[g].length);
for(var h=0;
h<PDC.ReviewsRatings.feedbackQueue[g].length;
h++){PDC.ReviewsRatings.feedbackQueue[g][h](j)
}}PDC.ReviewsRatings.feedbackQueue[g]=null
},error:function(j,h){pdclog("ratings-review/get-feedback error: "+h)
},failure:function(j,h){pdclog("ratings-review/get-feedback failure: "+h)
}})
}}};
/*!
...
if(rel&&rel!=="nofollow"){c=P("."+u).filter(function(){var aq=P.data(this,z);
var ar=new D(this,aq);
return(ar.get("rel")===rel)
});
O=c.index(al.el);
...
function f(ar){var aq;
if(!T){aq=P(ar).data("colorbox");
al=new D(ar,aq);
rel=al.get("rel");
Q();
if(!G){G=af=true;
...
a=U.outerWidth(true);
al.w=V(al.get("initialWidth"),"x");
al.h=V(al.get("initialHeight"),"y");
...
}}X.css({opacity:parseFloat(al.get("opacity"))||"",cursor:al.get("overlayClose")?"pointer":"",visibility:"visible"}).show();
if(al.get("closeButton")){B.html(al.get("close")).appendTo(d)
}else{B.appendTo("<div/>")
}ab()
}}function s(){if(!an&&m.body){ai=false;
ah=P(ad);
...
ao=M(ak,"Wrapper");
d=M(ak,"Content").append(q=M(ak,"Title"),x=M(ak,"Current"),Y=P('<button type="button"/>').attr({id:Z+"Previous"}),w=P('<button type="button"/>').attr({id:Z+"Next"}),ae=M("button","Slideshow"),h);
...
P(m.body).append(X,an.append(ao,l))
...
}if(av){if(typeof(av)=="function"){av()
}}},step:az})
};
...
}if(al.get("iframe")){ax=m.createElement("iframe");
if("frameBorder" in ax){ax.frameBorder=0
}if("allowTransparency" in ax){ax.allowTransparency="true"
}if(!al.get("scrolling")){ax.scrolling="no"
}P(ax).attr({src:al.get("href"),name:(new Date()).getTime(),"class":Z+"Iframe",allowFullScreen:true}).one("load",aw).appendTo(U);
L.one(j,function(){ax.src="//about:blank"
});
...
function ab(){var at,au,ar=S.prep,aq,av=++J;
af=true;
p=false;
K(j);
K(e);
...
if(al.get("inline")){aq=M(ak).hide().insertBefore(P(at)[0]);
L.one(j,function(){aq.replaceWith(U.children())
});
ar(P(at))
}else{if(al.get("iframe")){ar(" ")
}else{if(al.get("html")){ar(al.get("html"))
}else{if(F(al,at)){at=H(al,at);
p=m.createElement("img");
P(p).addClass(Z+"Photo").bind("error",function(){ar(M(ak,"Error").html(al.get("imgError")))
}).one("load",function(){var aw;
if(av!==J){return
}P.each(["alt","longdesc","aria-describedby"],function(ay,az){var ax=P(al.el).attr(az)||P(al.el).attr("data-"+az);
if(ax){p.setAttribute(az,ax)
}});
if(al.get("retinaImage")&&ad.devicePixelRatio>1){p.height=p.height/ad.devicePixelRatio;
p.width=p.width/ad.devicePixelRatio
}if(al.get("scalePhotos")){au=function(){p.height-=p.height*aw;
p.width-=p.width*aw
};
if(al.mw&&p.width>al.mw){aw=(p.width-al.mw)/p.width;
au()
}if(al.mh&&p.height>al.mh){aw=(p.height-al.mh)/p.height;
au()
}}if(al.h){p.style.marginTop=Math.max(al.mh-p.height,0)/2+"px"
}if(c[1]&&(al.get("loop")||c[O+1])){p.style.cursor="pointer";
p.onclick=function(){S.next()
}
}p.style.width=p.width+"px";
p.style.height=p.height+"px";
setTimeout(function(){ar(p)
},1)
});
setTimeout(function(){p.src=at
},1)
}else{if(at){l.load(at,al.get("data"),function(ax,aw){if(av===J){ar(aw==="error"?M(ak,"Error").html(al.get("xhrError")):P(this).contents())
}})
...
q.appendTo(m);
m.insertAfter(k);
d("html").live("mousedown",function(s){s.stopPropagation();
...
(function(f){var r=f("html"),q=f("body"),m=f(window),D=0,v=500,z=false,l=r.hasClass("touch"),d="80%",k="80%",s=false,y=false,j={};
var p=f(".ShowMobile").css("display")=="block"||f(".ShowTablet").css("display")=="block";
function g(E){return decodeURIComponent((new RegExp("[?|&]"+E+"=([^&;]+?)(&|#|;|$)").exec(location.search)||[,""])[1].replace(/\+/g,"%20"))||null
}function A(){var F=f("div#cboxPrevious");
var E=f("div#cboxNext");
f("div#colorbox").mouseover(function(){var G=f.colorbox.element();
if(!G.hasClass("lb_s7video")){F.stop().animate({left:"60px",opacity:1},300);
E.stop().animate({right:"60px",opacity:1},300)
}}).mouseout(function(){var G=f.colorbox.element();
if(!G.hasClass("lb_s7video")){F.stop().animate({left:"0",opacity:0},300);
E.stop().animate({right:"0",opacity:0},300)
}})
}function u(){f("#cboxClose").on("hover",function(E){f("#cboxClose").unbind("click");
f("#cboxClose").on("click",function(F){F.stopPropagation();
f("#cboxClose").addClass("cboxClose").removeAttr("id");
f.colorbox({scrolling:false,inline:true,maxWidth:"380px",height:"280px",href:"#confirm-dialog"})
})
})
}function o(){var E=f("#main-header").find("nav");
var F=E.find("audio");
var G=E.find(".nav-volume");
bAudio=F.length>0;
if(Modernizr.audio&&bAudio){if(G.hasClass("vol-on")){F[0].pause();
G.removeClass("vol-on")
}}}var x=function(E){E.preventDefault()
};
var a;
var h=function(){window.clearTimeout(a);
var E=function(){var F=f.colorbox.element();
if(F.hasClass("lb_youtube")||F.hasClass("lb_s7video")||F.hasClass("lb_kmjVideo")){return
}};
a=window.setTimeout(E,v)
};
var e=function(){o();
if(l){}m.on("resize.colorboxInit",h)
};
var C=function(){if(l){}m.off("resize.colorboxInit",h)
};
var w=g("lb");
function n(){z=swfobject.hasFlashPlayerVersion("9.0.2");
f.extend(f.colorbox.settings,{fixed:false,onOpen:e,onClosed:C,photo:true,scalePhotos:true,returnFocus:false,maxWidth:d});
if(!l){A()
}s=true
}function B(E){if(E.hasClass("lb_image")){E.colorbox({photo:true})
}if(E.hasClass("lb_imageWheel")){E.colorbox({photo:true,rel:"lb_imageWheel"})
}}function b(E){if(E.hasClass("lb_youtube")){E.colorbox({photo:false,iframe:false,innerWidth:d,innerHeight:k,maxWidth:false,maxHeight:false,html:function(){return E.getAgeGatedVideoHTML(E.getYTVideoDivHTML("ytVideoContainer"))
},onComplete:function(){E.playVideo(function(){f(".ytVideoContainer").showYTVideoPanel()
});
f(window).on("resize.pdcYTColorBox",function(){f("#colorbox").css("overflow","visible");
f.colorbox.resize({height:k,width:d})
});
f(window).trigger("resize.pdcYTColorBox")
},onClosed:function(){f(window).off("resize.pdcYTColorBox")
}});
E.updateYoutubeMetadata()
}if(E.hasClass("lb_s7video")){E.colorbox({photo:false,iframe:false,innerWidth:d,innerHeight:k,maxWidth:false,maxHeight:false,html:function(){return E.getAgeGatedVideoHTML(E.getVideoDivHTML("s7videoContainer"))
},onComplete:function(){E.playVideo(function(){if(!l){f(".s7videoContainer").showScene7VideoPanel()
}else{f(".s7videoContainer").s7NativeVideo()
}})
},onCleanup:function(){if(typeof onVideoPlayerClose=="function"){onVideoPlayerClose()
}}})
}if(E.hasClass("lb_kmjVideo")){if(z&&!l){E.colorbox({photo:false,iframe:false,innerWidth:d,innerHeight:k,maxWidth:false,maxHeight:false,html:function(){return f(this).getKMJVideoDivHTML("kmjVideoContainer")
},onComplete:function(){f(".kmjVideoContainer").showKMJVideoPanel()
}})
}else{E.nativeKmjVideo()
}}}function c(G){if(G.hasClass("lb_inline")){G.colorbox({scrolling:false,inline:true})
}if(G.hasClass("lb_inline_modal")){G.colorbox({scrolling:false,inline:true,width:"700px",height:"760px",maxWidth:"700px",maxHeight:"760px",overlayClose:true,onComplete:u})
}if(G.hasClass("lb_iframe")){var F={iframe:true,scrolling:true,width:"700px",height:"760px",maxWidth:"800px",maxHeight:"760px",overlayClose:true,fixed:true};
var E=false;
if(G.hasClass("link-btn")){if(p){E=true
}else{F.width="800px";
F.height="600px"
}}if(E){f(G).on("click",function(H){H.preventDefault();
window.open(this.href,"_blank")
})
}else{G.colorbox(F)
}}if(G.hasClass("lb_iframe_rtg")){var F={iframe:true,scrolling:false,width:"640px",height:"640px",overlayClose:true};
G.colorbox(F)
}if(G.hasClass("lb_inline_noscroll")){G.colorbox({inline:true,scrolling:false})
}}f.fn.pdcLightbox=function(){if(!y){return
}if(!s){n()
}f(this).each(function(){var E=f(this);
B(E);
b(E);
c(E)
});
...
f(document).ready(function(){y=true;
f(".lb_image,.lb_youtube,.lb_kmjVideo,.lb_s7video,.lb_inline,.lb_inline_modal,.lb_linline_noscroll,.lb_iframe,.lb_iframe_rtg,.lb_imageWheel").pdcLightbox();
if(w){f("#"+w).click()
}})
})(jQuery);
$(document).bind("cbox_cleanup",function(){console.log("-- cleanup");
$("#cboxClose").hide()
});
...
function e(){for(var k in options.texts){if(options.texts.hasOwnProperty(k)){console.log(k);
options.texts[k]=Adobe.I18n.get(options.texts[k])
}}d=true
...
OOo.K=function(){};
var E=E||OOo;
(function(){function k(a){return document.getElementById(a)
}function l(a,b){var c;
for(c in b){if(b.hasOwnProperty(c)){a[c]=b[c]
}}return a
}function o(a,b,c,d){if(a.addEventListener){a.addEventListener(b,c,d)
}else{if(a.attachEvent){a.attachEvent("on"+b,c)
}}}function m(a,b,c,d){if(a.removeEventListener){a.removeEventListener(b,c,d)
}else{if(a.detachEvent){a.detachEvent("on"+b,c)
}}}function s(a){var b=[],c;
for(c in a){if(a.hasOwnProperty(c)){b.push(c+"="+(encodeURIComponent(a[c])||""))
}}return b.join("&")
}function n(a){var b=s(a.metrics),c=a.tealeafId+"|"+a.clickTalePID+"/"+a.clickTaleUID+"/"+a.clickTaleSID;
...
}function q(a,b){var c=document,d=c.createElement("form"),e=c.createElement("input"),f=a.referrerRewrite;
...
if(a.commentCardUrl){d.action=a.commentCardUrl;
...
return d
}function t(){return{width:screen.width,height:screen.height,referer:location.href,prev:document.referrer,time1:(new Date()).getTime(),time2:null,currentURL:location.href,ocodeVersion:"5.8.4"}
}function p(a){var b="";
if(a&&a.search("://")>-1){var c=a.split("/");
for(var d=3;
d<c.length;
d++){b+="/";
b+=c[d]
...
}function v(a,b){if(!b){b=location
}if(typeof a==="string"){return a
}return a.searchPattern?b.href.replace(a.searchPattern,a.replacePattern):a.replacePattern
}function w(a){var b,c=false,d=document.getElementsByTagName("meta");
if(d!=="undefined"){for(b=0;
b<d.length;
b+=1){if(d[b].getAttribute("name")===a){c=true
}}}return c
}var u=(function(){if(navigator.appName==="Microsoft Internet Explorer"&&navigator.userAgent.search("MSIE 6")!==-1){return true
}var a=document.body,b,c;
if(document.createElement&&a&&a.appendChild&&a.removeChild){b=document.createElement("iframe");
c=false;
...
}function C(){var a=OOo.$("oo_invitation_prompt");
...
c.send(null)
}function x(a,b){var c=document,d=typeof a==="string"?c.createElement("div"):a,e=c.createElement("div"),f,g,h=this.options,i;
e.id="oo_invitation_overlay";
d.id="oo_container";
d.style.visibility="hidden";
if(typeof a==="string"){d.innerHTML=a;
c.body.appendChild(d)
}d.appendChild(e);
i=OOo.$("oo_launch_prompt");
if(h.companyLogo){f=new Image();
f.src=h.companyLogo;
...
if(h.clickCallbacks){if(typeof h.clickCallbacks.yes==="function"){OOo.addEventListener(i,"click",function(){h.clickCallbacks.yes()
},false)
}if(typeof h.clickCallbacks.no==="function"){OOo.addEventListener(OOo.$("oo_no_thanks"),"click",function(){h.clickCallbacks.no()
},false)
}}if(OOo.Browser.IE&&!window.XMLHttpRequest){e.style.position="absolute";
...
(function(){function f(a){if(!a){return null
}switch(typeof a){case"number":case"boolean":case"function":return a;
case"string":return"'"+a+"'";
case"object":var b,c,d,e;
if(a.constructor===Array||typeof a.callee!=="undefined"){b="[";
d=a.length;
for(c=0;
c<d-1;
c+=1){b+=f(a[c])+","
...
for(e in a){if(a.hasOwnProperty(e)){b+=e+":"+f(a[e])+","
}}b=b.replace(/\,$/,"")+"}"
}return b;
default:return null
}}OOo.extend(OOo,{serialize:f})
}());
(function(){function e(a,b,c){var d;
if(a.search(b[0])!==-1){OOo.createCookie(c,0);
return false
}else{if(OOo.readCookie(c)){d=parseInt(OOo.readCookie(c),10);
if((a.search(b[d+1])!==-1)&&(d+1!==b.length-1)){OOo.createCookie(c,d+1);
return false
}else{if(a.search(b[d])!==-1){return false
}else{if(d+1===b.length-1&&a.search(b.pop())!==-1){OOo.eraseCookie(c);
return true
}else{OOo.eraseCookie(c);
return false
}}}}else{return false
}}}OOo.extend(OOo,{checkTunnel:e})
}());
(function(){function q(a){var b="",c;
for(c=7;
c>=0;
c-=1){b+="0123456789abcdef".charAt((a>>(c*4))&15)
}return b
}function t(a){var b=((a.length+8)>>6)+1,c=new Array(b*16),d;
for(d=0;
d<b*16;
d+=1){c[d]=0
}for(d=0;
d<a.length;
d+=1){c[d>>2]|=a.charCodeAt(d)<<(24-(d%4)*8)
}c[d>>2]|=128<<(24-(d%4)*8);
c[b*16-1]=a.length*8;
return c
}function p(a,b){var c=(a&65535)+(b&65535),d=(a>>16)+(b>>16)+(c>>16);
return(d<<16)|(c&65535)
}function r(a,b){return(a<<b)|(a>>>(32-b))
}function v(a,b,c,d){if(a<20){return(b&c)|((~b)&d)
}if(a<40){return b^c^d
}if(a<60){return(b&c)|(b&d)|(c&d)
}return b^c^d
}function w(a){return(a<20)?1518500249:(a<40)?1859775393:(a<60)?-1894007588:-899497514
}function u(a){var b=t(a),c=new Array(80),d=1732584193,e=-271733879,f=-1732584194,g=271733878,h=-1009589776,i,j,k,l,o,m,s,n;
for(s=0;
s<b.length;
s+=16){i=d;
j=e;
...
for(n=0;
n<80;
n+=1){if(n<16){c[n]=b[s+n]
}else{c[n]=r(c[n-3]^c[n-8]^c[n-14]^c[n-16],1)
...
(function(){function h(a,b){if(!b){b=location
...
}else{if(b.pathname.indexOf(f)!==-1){OOo.eraseCookie(c);
return false
}else{if(b.pathname.search(g)!==-1){return false
}else{OOo.eraseCookie(c);
return true
}}}}OOo.extend(OOo,{checkAbandonment:h})
}());
(function(){function d(a){var b,c;
for(b=a.length-1;
b>=0;
b-=1){if(a[b].read){c=OOo.readCookie(a[b].name);
if(!!c&&c===a[b].value){return true
}else{if(typeof a[b].value==="undefined"&&!!OOo.readCookie(a[b].name)){return true
}}}}return false
}function e(a){var b;
for(b=a.length-1;
b>=0;
b-=1){if(a[b].set){OOo.createCookie(a[b].name,a[b].value,a[b].expiration)
...
function f(a,b){var c=a.length,d=b.length;
while(d){d-=1;
a[c+d]=b[d]
}return a
}function g(a,b){a=e.call(a,0);
return f(a,b)
}function h(b){if(arguments.length<2&&typeof b==="undefined"){return this
}var c=this,d=e.call(arguments,1);
return function(){var a=g(d,arguments);
return c.apply(b,a)
}
}return{bind:h}
}()));
(function(){function f(a){if(!a){a=location
}var b;
if(a.host.search(/\.[a-z]+/)!==-1){b=a.host.split(".").reverse();
if(b.length>3){return a.host
}b="."+b[1]+"."+b[0]
}else{b=a.host
}return b
}function g(a,b,c){var d="",e="";
if(c){d=new Date();
d.setTime(d.getTime()+(c*1000));
e="; expires="+d.toGMTString()
}if(location.host!==f()){document.cookie=a+"="+b+e+"; path=/; domain="+f()+";"
}else{document.cookie=a+"="+b+e+"; path=/;"
}}function h(a){var b=a+"=",c=document.cookie.split(";"),d,e;
for(e=0;
e<c.length;
e+=1){d=c[e];
...
OOo.Ocode=function(a){var b=OOo.Browser,c,d;
if(a.disableMobile&&b.isMobile){return
}if(a.disableNoniOS&&b.isMobileNonIOS){return
}OOo.instanceCount+=1;
...
if(c.cookie&&OOo.Ocode.matchUrl(c.cookie,location)){return
...
if(c.events){this.setupEvents();
if(c.events.disableLinks||c.events.disableFormElements){this.setupDisableElements()
}}if(c.floating){this.floating()
}else{if(c.bar){this.bar()
}else{if(c.tab){this.tab()
}}}};
OOo.Ocode.prototype={show:function(a,b){if(a==="Tab"&&b&&b.preventDefault){b.preventDefault()
}if(this.onPageCardVisible){return
}var c=this.options,d;
if(c.events&&c.events.prompt){if(c.cookie){OOo.eraseCookie(c.cookie.name||"oo_r")
}OOo.hidePrompt()
...
}if(c.thirdPartyCookies){if(OOo.checkThirdPartyCookies(c.thirdPartyCookies)){return
...
}if(c.clickTalePID&&typeof window.ClickTale==="function"){c.clickTaleUID=window.ClickTaleGetUID();
...
OOo.extend(OOo.Ocode,{tagUrl:function(a,b){if(!b){b=location
}var c=a.name||"oo_r",d=a.type==="page"?b.href:b.hostname,e=OOo.readCookie(c)||"";
if(OOo.Ocode.matchUrl(a,b)){return
}OOo.createCookie(c,e+OOo.sha1(d),a.expiration)
},matchUrl:function(a,b){if(!b){b=location
...
(function(){var i=0;
function j(){var a=this.options,b=a.newWindowSize||[545,325],c=[parseInt((a.metrics.height-b[1])/2,10),parseInt((a.metrics.width-b[0])/2,10)],d,e,f="resizable=yes,location=no,status=no,scrollbars=1,width="+b[0]+",height="+b[1]+",top="+c[0]+",left="+c[1],g="OnlineOpinion";
...
}if(OOo.Browser.isMobile&&OOo.Browser.ua.search("Android")!==-1){d.submit()
}else{e=window.open(h,g,f);
if(e&&!OOo.Browser.isChromeIOS){d.submit()
}}}OOo.extend(OOo.Ocode.prototype,{launchOOPopup:j})
}());
(function(){function j(){var a=this.options.events,b=[false,false],c=["onExit","onEntry"],d="beforeunload",e,f,g,h,i;
...
}if(b[1]){if(a.delayEntry){window.setTimeout(function(){if(a.prompt){this.getPrompt()
}else{this.show()
}}.bind(this,"onEntry"),a.delayEntry*1000)
}else{if(a.prompt){this.getPrompt()
}else{this.show("onEntry")
}}}}function k(a){var b=a||window.event,c=a.target||a.srcElement,d=this.options.events,e=c.parentNode,f=5,g=0;
while(e&&(c.nodeName!=="A"||c.nodeName!=="INPUT")&&g!==f){if(e.nodeName==="A"){c=e
}e=e.parentNode;
g+=1
...
}function o(){OOo.addEventListener(document.body,"mousedown",k.bind(this));
if(!this.options.events.disableFormElements){return
}var a=document.getElementsByTagName("form"),b;
for(b=a.length-1;
b>=0;
b-=1){OOo.addEventListener(a[b],"submit",l.bind(this))
}}OOo.extend(OOo.Ocode.prototype,{setupEvents:j,setupDisableElements:o,getPrompt:function(){OOo.getPrompt.call(this)
},showPrompt:function(a){if(this.options.cookie){OOo.Ocode.tagUrl(this.options.cookie)
}OOo.showPrompt.call(this,a,this.show)
}})
}());
OOo.extend(OOo.Ocode.prototype,{floating:function(){var d=document,e=this.floatingLogo=document.createElement("div"),f=d.createElement("div"),g=d.createElement("div"),h=d.createElement("div"),i=d.createElement("span"),j=this.options.floating,k=OOo.$(j.contentId),l="10px",o=j.id,m=d.createElement("span"),s,n,q,t,p,r,v,w;
...
h.className="olOver";
f.tabIndex=0;
f.onkeyup=function(a){s=a||window.event;
...
e.appendChild(h);
e.appendChild(g);
function C(a){var b=d.documentElement.scrollTop||d.body.scrollTop,c=d.documentElement.clientHeight||document.body.clientHeight;
e.style.top=(b+c-(v||0)-10)+"px"
}if(OOo.Browser.MobileSafari){if(OOo.Browser.ua.search("OS 4")!==-1){n=window.innerHeight;
e.style.bottom=null;
e.style.top=(window.pageYOffset+window.innerHeight-60)+"px";
w=function(a){q=window.pageYOffset-(n-window.innerHeight);
e.style.webkitTransform="translateY("+q+"px)"
};
OOo.addEventListener(window,"scroll",w,false);
setTimeout(w,100)
}}if(j.position&&j.position.search(/Content/)&&k){t=this.spacer=d.createElement("div");
p=OOo.Browser.WebKit?d.body:d.documentElement;
t.id="oo_feedback_fl_spacer";
t.style.left=u(k)+"px";
d.body.appendChild(t);
switch(j.position){case"rightOfContent":r=function(a){e.style.left=(u(k)-p.scrollLeft)+"px"
};
break;
case"fixedPreserveContent":r=function(a){var b=OOo.Browser.IE?d.body.clientWidth:window.innerWidth,c=p.scrollLeft;
if(b<=u(k)+e.offsetWidth+parseInt(l,10)){e.style.left=(u(k)-c)+"px"
}else{e.style.left="";
e.style.right=l
}};
break;
case"fixedContentMax":r=function(a){var b=OOo.Browser.IE?d.body.clientWidth:window.innerWidth;
if(b<=u(k)+e.offsetWidth+parseInt(l,10)){e.style.left="";
e.style.right=l
}else{e.style.left=(u(k)-p.scrollLeft)+"px";
e.style.right=""
}};
break
...
OOo.extend(OOo.Ocode.prototype,{bar:function(){var d=document,e=this.floatingLogo=d.createElement("div"),f=d.createElement("span"),g,h,i,j=d.documentElement.scrollTop||d.body.scrollTop,k=d.createElement("div");
function l(a){var b=0,c=0;
if(a.offsetParent){do{b+=a.offsetLeft;
c+=a.offsetTop
}while(a==a.offsetParent);
return[b,c]
}}function o(a){var b=document.activeElement,c;
if(!b){return
}c=l(b);
if(!c){return
}if(c[1]+b.clientHeight>(window.innerHeight||document.body.clientHeight)+(window.pageYOffset||document.body.scrollTop)-e.clientHeight){if(navigator.appVersion.indexOf("MSIE 7.")!==-1){window.scrollBy(0,0)
}else{window.scrollBy(0,b.clientHeight+20)
...
e.appendChild(f);
e.tabIndex=0;
e.onkeyup=function(a){var b=a||window.event;
...
if(OOo.Browser.IE&&m&&+m[1]<8){if(d.compatMode==="CSS1Compat"){g=function(a){if(a&&a.type==="resize"){setTimeout(g,50)
...
}else{g=function(a){e.style.top=(d.body.scrollTop+document.body.clientHeight-e.clientHeight-1)+"px";
...
g()
}else{if(OOo.Browser.MobileSafari&&OOo.Browser.ua.search("OS 4")!==-1){h=window.innerHeight;
e.style.bottom=null;
...
OOo.extend(OOo.Ocode.prototype,{tab:function(){var e=document,f=this.floatingLogo=e.createElement("div"),g=e.createElement("div"),h=e.createElement("div"),i=e.createElement("span"),j=this.options.tab,k=e.createElement("a"),l="Feedback",o=j.tabType,m="right";
if(j.wcagBasePath){h=e.createElement("a");
h.setAttribute("href","#");
i=e.createElement("img");
i.className="logo";
i.setAttribute("alt","Feedback");
i.setAttribute("src",j.wcagBasePath+((OOo.Browser.ua.search("IE 6")!==-1)?"oo_tabie6.png":"oo_tab.png"))
}function s(a){var b=e.documentElement.scrollTop||e.body.scrollTop,c=e.documentElement.scrollLeft||e.body.scrollLeft,d=e.documentElement.clientHeight||document.body.clientHeight;
f.style.top=(b+(d/2-f.clientHeight/2))+"px";
if((!j.position||j.position==="right")){f.style.right=(-1*c+2)+"px"
}}function n(a){f.style.top=pageYOffset+(innerHeight/2-f.clientHeight/2)+"px";
...
if(typeof o==="number"&&o>=0){f.id="oo_tab_"+o;
if(j.position!==undefined){m=j.position
}f.className="oo_tab_"+m+"_"+o;
if(j.verbiage!==undefined){l=j.verbiage
}switch(o){case 1:k.id="oo_tab_text_"+o;
k.className=m;
k.innerHTML=l;
if(m==="left"){k.className=j.position;
if(OOo.Browser.isMobile||OOo.Browser.iOs){k.style.left="5px"
}}break
}}if(j.wcagBasePath){f.className+=" wcag"
}if(typeof j.tabIndex==="number"){f.tabIndex=j.tabIndex
}else{if(typeof j.tabIndex==="undefined"){f.tabIndex=0
}}f.onkeyup=function(a){var b=a||window.event;
if(b.keyCode!==13){return
}this.show()
}.bind(this);
h.appendChild(k);
...
h.appendChild(i);
f.appendChild(h);
if(g){g.className="screen_reader";
...
return false
}x=OOo.Ocode.postMessageHandler(function(a){var b=parseInt(a,10),c,d;
if(b>0){if(r){return
}r=true;
...
h.tabIndex=0;
h.onkeyup=function(a){var b=a||window.event;
if(b.keyCode!==13){return
}y()
};
if(OOo.Browser.IE){l.frameBorder="0";
if(!window.XMLHttpRequest||e.compatMode==="BackCompat"){D=Math.max(e.documentElement.clientWidth,e.body.offsetWidth);
...
if(s.closeWithOverlay&&!OOo.Browser.isMobile){g.appendChild(j);
j.onclick=y;
f.onclick=y
}l.src=" ";
l.name=k;
...
}else{t=setInterval(x,500)
}m.metrics.time2=(new Date()).getTime();
q=OOo.appendOOForm(m,k);
q.submit()
}});
OOo.extend(OOo.Ocode,{postMessageHandler:function(d,e,f){return function(a){var b="https://secure.opinionlab.com",c;
if(!f){f=location
}if((a&&!(a.origin===b||a.origin.indexOf(e)!==0))||(!a&&f.hash.search("OL=")===-1)){return false
...
}if(window.OOoDynamicRewrite&&("number"===typeof window.OOoDynamicRewrite.weight)&&!isNaN(window.OOoDynamicRewrite.weight)){OOo.createCookie("oo_OODynamicRewrite_weight",window.OOoDynamicRewrite.weight);
...
}}if(location.search.search("evs")!==-1||OOo.readCookie("oo_evs_friendly")==="yes"){OOo.eraseCookie("oo_evs_friendly");
...
}if(b.area&&location.href.search(b.area)===-1){this.options.popupType="popup";
this.launchPopup()
}else{if(b.goal&&location.href.search(b.goal)!==-1){window.oo_inv_monitor.close()
}}}.bind(this),1600);
if(OOo.readCookie(b.repromptCookie)){return
}if(b.thirdPartyCookies&&OOo.checkThirdPartyCookies(b.thirdPartyCookies)){return
}if(!OOo.readCookie(b.percentageCookie)){OOo.createCookie(b.percentageCookie,(Math.random()>1-(b.responseRate/100))?"1":"0")
}if(typeof b.promptTrigger!=="undefined"){if(b.promptTrigger instanceof RegExp){if(!window.location.href.match(b.promptTrigger)){return
}}else{if(b.promptTrigger instanceof Array){if(!OOo.checkTunnel(location.pathname,b.promptTrigger,b.tunnelCookie)){return
}}}}c+=1;
OOo.createCookie(b.pagesHitCookie,c);
if(b.pagesHit&&c<b.pagesHit){return
}OOo.eraseCookie(b.tunnelCookie);
if(OOo.readCookie(b.percentageCookie)==="1"){window.setTimeout(function(){OOo.createCookie(b.repromptCookie,1,b.repromptTime);
this.options.beforePrompt();
this.getPrompt()
}.bind(this),b.promptDelay*1000)
}};
OOo.Invitation.notifyFriendlyLocationChange=function(a){if(window.oo_inv_monitor){OOo.createCookie("oo_evs_friendly","yes")
}};
...
},launchPopup:function(){if(this.popupShown){return
}this.popupShown=true;
var b=this.options,c=window.location.href,d=b.popupType==="popup"?"https://secure.opinionlab.com/ccc01/comment_card.asp?":b.pathToAssets+b.monitorWindow+"?time1="+(new Date()).getTime()+"&",e,f=[],g=b.asm?[555,500]:[400,335],h,i=OOo.createMetrics(),j=OOo.readCookie(b.tealeafCookieName),k;
...
k="scrollbars=1,resizable=1,location=no,status=no,width="+g[0]+",height="+g[1];
if(b.referrerRewrite){i.referer=OOo.referrerRewrite(b.referrerRewrite)
}if(b.truncatePrevCurrentMetrics){i.prev=OOo.truncateMetric(i.prev);
i.currentURL=OOo.truncateMetric(i.currentURL)
}if(b.disablePrevCurrentMetrics){i.prev="";
i.currentURL=""
...
}if(b.popupType==="popunder"){if(!OOo.Browser.Chrome){h.blur();
window.focus()
}else{if(!b.loadPopupInBackground){window.alert(b.chromeMainWinPrompt||"Please fill out the form behind this window when you are finished.")
}if(b.chromeSurveyPrompt){setTimeout(function(a){h.postMessage(b.chromeSurveyPrompt,"*")
},500)
}}}else{if(window.oo_inv_monitor){if(!OOo.Browser.Chrome){window.blur();
h.focus()
}else{h.alert(b.chromeSurveyPrompt||"Please fill out the form");
h.focused=true
}}}},killPrompt:function(){if(this.options.clickCallbacks&&typeof this.options.clickCallbacks.no==="function"){this.options.clickCallbacks.no()
...
var PDC=PDC||{};
PDC.UserNotificatio={getNotifications:function(b){if(document.location.href.indexOf("en-us")>0){if(document.location.protocol==="https:"){b=b.replace("http://","https://");
b=b.replace("Http://","https://");
if(b.indexOf("pre-prd")>0){b=b.replace("pre-prd","secure.preprd")
}else{b=b.replace("https://","https://secure.")
}}try{var c=new RegExp("userinfo=([^;]+)");
var g=c.exec(document.cookie);
var f=(g!=null)?unescape(g[1]):null;
if(f!=null){var a=b.split("userInfoCookie=");
b=a[0]+"userInfoCookie="+f
}var d=$.getJSON(b,function(e){}).done(function(l){try{var j=l.totalCampaigns;
var k=$(".show-social #notification");
$.each(k,function(e,n){$(n).empty().append(tmpl("notification-list",l))
});
if(!$(".avatar a.nav-content .notification-count").length){if(parseInt(j)>0){$(".avatar a.nav-content span").append(tmpl("notification-count",l))
}}}catch(m){}});
$.when(d).fail(function(e,k,j){})
}catch(h){console.log("notification call fails")
}}}};
(function(e,c,a,g){var d="platinumTrophyAchieversOverlay",f={getUeserMarque:"https://pre-prd.us.playstation.com/playstation/psn/users/marquee",getPlatinumTrophyURL:"https://secure.preprd.us.playstation.com/playstation/psn/save/marquee",updatePlatinumTrophyURL:"https://secure.preprd.us.playstation.com/playstation/psn/users/marquee/read"};
...
var j=PDC.Profile.isLoggedIn();
if(j){h.sendRequest()
}};
b.prototype.getCokieUser=function(k){var h=new RegExp("[; ]"+k+"=([^\\s;]*)");
var j=(" "+a.cookie).match(h);
if(k&&j){return unescape(j[1])
}return""
};
b.prototype.sendRequest=function(){var h=this;
var j=h.options.getUeserMarque+"?userInfoCookie="+h.getCokieUser("userinfo");
e.getJSON(j,function(){}).done(function(k){h.buildMarqueeDom(k);
e(".cboxElement2").colorbox({photo:true,scalePhotos:true,maxWidth:(e(c).width())-50,rel:"cboxElement2",returnFocus:false,maxHeight:(e(c).height())-50});
e("#marquee0 a").click();
(function(o,l,p){var n,m=o.getElementsByTagName(l)[0];
...
n.id=p;
n.src="//connect.facebook.net/en_US/all.js#xfbml=1&appId=5437122869";
m.parentNode.insertBefore(n,m)
}(a,"script","facebook-jssdk"));
c.twttr=(function(p,m,q){var l,o,n=p.getElementsByTagName(m)[0];
if(p.getElementById(q)){return
}o=p.createElement(m);
o.id=q;
o.src="https://platform.twitter.com/widgets.js";
n.parentNode.insertBefore(o,n);
return c.twttr||(l={_e:[],ready:function(r){l._e.push(r)
}})
}(a,"script","twitter-wjs"));
e(a).bind("cbox_complete",function(){var p=e(".cboxPhoto").attr("src").split("?id=")[1];
p=p.split("&cls=");
var l=p[1],p=p[0];
var n=PDC.Profile;
var q=n.getOnlineID();
var m=q;
var o=h.options.getPlatinumTrophyURL+"?onlineId="+m+"&marqueeId="+parseInt(p);
e.post(h.options.updatePlatinumTrophyURL+"?marqueeId="+p,function(r){}).done(function(r){});
e(".mainSocial").remove();
e("#cboxContent").append('<div class="mainSocial '+l+'"><div class="backgroundimg"></div><div  id="fbButton"><div id="fb-root"></div><div class="fb-share-button" data-layout="button" data-width="100%" data-href="'+o+'"></div></div><div id="twButton"><a href="https://twitter.com/share" class="twitter-share-button"  data-url="'+o+'"   data-text="Here is my PlayStation platinum trophy achievement!!!" data-lang="en"  data-count="none">Tweet</a></div></div>');
twttr.widgets.load();
FB.XFBML.parse()
})
})
};
b.prototype.buildMarqueeDom=function(l){var j=this;
var m=l;
for(var h in m){var k=e("<div> </div>").addClass("noShowMarquee").css("display","none").prependTo("body");
e.each(m[h],function(n,p){_className=(p.generic===0)?"specific":"generic";
var q=e("<div> </div>").appendTo(".noShowMarquee").addClass("image-block othermod-lft").attr("id","marquee"+n);
var o=e("<a> </a>").addClass("clearfix cboxElement2 ply-btn").attr("href",p.marqueeUrl+"?id="+p.marqueeId+"&cls="+_className).appendTo(q);
e("<div> </div>").attr("class","button gdpimagecarosal").text(p.marqueeId).appendTo(o);
e("<img/>").attr({"data-id":p.marqueeId,src:p.marqueeUrl,alt:"",title:""}).appendTo(o)
})
}};
...
$(document).ready(function(){PDC.Profile.psPlus="true";
pdclog("psplus: member? ",PDC.Profile.getPsPlus());
if(PDC.Profile.getPsPlus()!=="true"){return
}var URL="psplus.member.url";
var LABEL="psplus.member.label";
$("a.x-psplus-toggle").each(function(){var $a=$(this);
var data={};
try{eval("data = "+$a.data("sc"));
if(data[URL]){$a.attr("href",data[URL])
}if(data[LABEL]){$a.find("div").html(data[LABEL])
}}catch(e){}})
});
    ...
    return element.each(function(){
        ...
        function tidy() {
          ...
          root.find(defaults.content_class + ' .row').each(function(i) {
            ...
            var parent = $(this).parents('.unit-item');

            if (
              !parent.hasClass('field_demographics_unit') &&
              !parent.hasClass('field_metrics_unit') &&
              !parent.hasClass('field_distribution') &&
              !parent.hasClass('field_opportunities_by_device')
            ) {
  ...
  Plugin.prototype = {
    ...
    open: function(content) {
      ...
      if ($('.carousel', active).length > 0) {
        var carousel = $('.carousel', active);
        heights = this.getHeights(item, carousel);
        carousel.carousel({animateFirst: false, maxHeight: heights.maxHeight});
      }
      else if ($('#image', active).length > 0) {
        var image = $('#image', active);
        heights = this.getHeights(item, image);
        image.heritageImage({maxHeight: heights.maxHeight});
      }

      addthis.button('.addthis_button');

      active.css('height', this.options.listItemHeight).animate({'height': heights.height}, this.options.duration, 'easeOutCirc', function() {
    ...
    resize: function(e) {
      var item = $('.item.active #item', this.element);

      if ($('.carousel', item).length > 0) {
        var carousel = $('.carousel', item);
        var heights = this.getHeights(item, carousel);
        carousel.data('plugin_carousel').updateMaxHeight(heights.maxHeight);
      }
      else if ($('#image', item).length > 0) {
        var image = $('#image', item);
        var heights = this.getHeights(item, image);
        image.data('plugin_heritageImage').updateMaxHeight(heights.maxHeight);
      }
    },

    getHeights: function(item, mediaElement) {
      var height = Math.min(item.data('height'), $(window).height() - 42);
      var contentHeight = item.outerHeight() - mediaElement.outerHeight();
      var maxHeight = height - contentHeight;
      return {
        height: height,
        maxHeight: maxHeight
      };
...
;(function ($, window, document, undefined) {
  ...
  Plugin.prototype = {
    ...
    init: function() {
      this.updateHeight();
      $('img', this.element).load((function(e) {
        this.updateHeight();
      }).bind(this));
    },
    ...
    updateHeight: function() {
      ...
      var height = Math.min(this.element.height(), this.options.maxHeight);

      var caption = $('.caption', this.element);
      ...
      var height = this.options.maxHeight - (caption.outerHeight() + 9);
      width = Math.round(img.width() * (height / img.height()));
      
      if (width <= parseInt(img.css('max-width').replace('px', ''))) {
        img.height(height).width(width).attr({width: width, height: height});
      }
    },
...
;(function ($, window, document, undefined) {
  ...
  Plugin.prototype = {
    ...
    updateHeight: function(animate) {
      ...
      var height = Math.min($(this.items[this.index]).height(), this.options.maxHeight);

      var opts = {height: height};
      if (this.options.animateFirst && animate) {
      ...
      this.updateItemHeight(this.items[this.index]);
    },
    ...
    updateItemHeight: function(item) {

      if (!this.options.maxHeight) {
        return false;
      }

      $(item).css('max-height', this.options.maxHeight);
  ...
  Plugin.prototype = {
    ...
    initCarousel: function() {
      ...
      $(this.data.items).each((function(i, item) {
        ...
        // if something for some reason is wrong with the json
        if (item) {
          var data = item;

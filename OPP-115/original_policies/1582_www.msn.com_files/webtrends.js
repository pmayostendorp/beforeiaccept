...
function WebTrends(){
	...
	//Updated from TB 8.6 to handel reserved charicters in query string
...
WebTrends.prototype.dcsJSONTrack=function(jperm, jtemp)
{
    for (var i in jperm) {
    //Set Param
        if (typeof(i)!="undefined" && typeof(jperm[i])!="undefined") {
            var Args = new Array();
            Args[0]=i.toString();
            Args[1]=jperm[i].toString();
            this.dcsSetProps(Args);
        }
    }
    for (var i in jtemp) {
    //Set Param
        if (typeof(i)!="undefined" && typeof(jtemp[i])!="undefined") {
            var Args = new Array();
            Args[0]=i.toString();
            Args[1]=jtemp[i].toString();
            this.dcsSetProps(Args);
            //Add to Clear Var
            this.DCSdir.ClearVars = this.DCSdir.ClearVars+","+i;
        }
    }
    //reset date stamp and make L req
    var dCurrent=new Date();
	this.DCS.dcsdat=dCurrent.getTime();
	this.dcsFPC();
	this.dcsTag();
...
WebTrends.prototype.dcsFormButton=function(evt){
	...
	if (evt&&((typeof(evt.which)!="number")||(evt.which==1))){
		...
		for (var j=0;j<tags.length;j++){
			...
			var type=e.type||"";
			if (type&&((type=="submit")||(type=="image")||(type=="button")||(type=="reset"))||((type=="text")&&((evt.which||evt.keyCode)==13)))if (!this.dcsMSLinkTrackException(e)){
				var uri="";
				var ttl="";
				var id=0;
				if (e.form){
				...
				if (uri&&ttl&&(evt.keyCode!=9)){
					this.dcsMultiTrack("DCS.dcsuri",uri,"WT.ti","FormButton:"+ttl,"WT.dl",1,"WT.nv",this.dcsNavigation(evt),"DCSext.wtNavigation",this.dcsNavigation(evt));
				}
...
WebTrends.prototype.dcsCollect=function(){
    if (this.enabled){
        ...
        this.dcsMeta();
        this.dcsAdv();
        //this.dcsTag(); // Removed so homepage can send a custom page load beacon
    }
}
...
function dcsEscape(S){
      if (typeof(_tag)!="undefined"){
            return(_tag.dcsEscape(S));
      }
}

function dcsEncode(S){
      if (typeof(_tag)!="undefined"){
            return(_tag.dcsEncode(S));
      }
}
...
WebTrends.prototype.dcsMSVar=function(){
	...
	if (typeof(this.WT.z_gTrackExceptions)!="undefined") {
		this.DCSdir.gTrackExceptions = this.WT.z_gTrackExceptions;
	}
	...
	if (typeof(this.WT.sp)!="undefined") {
	    this.WT.sv_sp=this.WT.sp;
	        if (typeof(this.DCSext.wt_pt)!="undefined") {
	            this.DCSext.wt_sppt=this.WT.sp+this.DCSext.wt_pt;
	            this.DCSext.wt_sppt = this.DCSext.wt_sppt.toLowerCase();
	        }
	}
}
...
WebTrends.prototype.dcsMSLinkTrack=function(evt){
	...
	if (evt&&((typeof(evt.which)!="number")||(evt.which==1))){
		...
		if (e.href&&e.protocol&&e.protocol.indexOf("http")!=-1&&!this.dcsMSLinkTrackException(e)){		    
		    //GDH 3.2.1 Added to deal with None IE dcsSetVar+Click Tracking onclick or onmousedown
...
WebTrends.prototype.dcsMSImageMap=function(evt) {
    ...
    if (evt && (typeof evt.which != "number" || evt.which == 1)) {
        ...
        if (e.href&&e.protocol&&e.protocol.indexOf("http")!=-1&&!this.dcsMSLinkTrackException(e)){
        	...
        	this.WT.sp=this.WT.ad=this.DCS.setvar_dcsuri=this.DCS.setvar_dcssip=this.DCS.setvar_dcsqry=this.WT.setvar_ti=this.WT.setvar_mc_id="";
  	        if (e.attributes.getNamedItem("cid")) this.DCSext.wt_linkid=e.attributes.getNamedItem("cid").value;
        	this.dcsMultiTrack("DCS.dcssip",hn,"DCS.dcsuri",pth,"DCS.dcsqry",this.trimoffsiteparams?"":qry,"DCS.dcsref",window.location,"WT.ti","Img Map:"+ti,"WT.dl","1","WT.nv",this.dcsNavigation(evt),"DCSext.wtNavigation",this.dcsNavigation(evt),"WT.sp","","WT.ad","");
        	this.DCS.dcsref=this.WT.ti=this.WT.dl=this.WT.nv="";
        }
    }
}

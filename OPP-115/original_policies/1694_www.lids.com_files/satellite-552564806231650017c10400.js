// retroactive compatibility for old Tag Manager code
var amc = {};
amc.on = function(type,cb){
  if(type=='load'){
    cb();
  }
}
...
var sc = {  sites: [
  { name: 'lids us', domain: 'www.lids.com', account: 'hwlprod' },
  { name: 'lids us custom', domain: 'custom-hats.lids.com', account: 'hwlprod' },
  { name: 'lids us mobile', domain: 'm.lids.com', account: 'hwlprod' },
  { name: 'lids ca', domain: 'www.lids.ca' , account: 'hwlhwlcaprod' },
  { name: 'lids ca custom', domain: 'custom-hats.lids.ca', account: 'hwlhwlcaprod' },
  { name: 'lids ca mobile', domain: 'm.lids.ca', account: 'hwlhwlcaprod' },
  { name: 'kiosk'  , domain: 'k.lids.com'  , account: 'hwlkiosk' }
],
          getAccount: function(){
            for( var i=0; i<this.sites.length; i++ ){
              if( location.hostname.toLowerCase() == this.sites[i].domain.toLowerCase() ) {
                return this.sites[i].account+',hwlglobal';
              }
            }
            return 'hwldev';
          }
...
/* Internal Campaign Impressions */
var IntCamImp = function(){
  if ($('#webpage').is('*')) {
    var slist1 = '';
    var linkids = $('#webpage').html().toString().match(/link_id=[^&"']+/gi);

    if (linkids) {
      var linkid_cache = {}; 

      $.each(linkids,function(k,v){
        var linkid = v.replace('link_id=','');

        if (!linkid_cache[linkid.toLowerCase()]) { 
          if (slist1 != '') { slist1 = slist1 + ','; }
          slist1 = slist1 + linkid;
        }

        linkid_cache[linkid.toLowerCase()] = linkid;
      });
    }
...
/* Internal Campaign Impressions src_id */
var recsTitles = function(){
  if ($('#productpage').is('*')) {
    var slist2 = '';
    $('div#recs img').each(function() {
      if (slist2 != '') { slist2 = slist2 + ','; }
      slist2 = slist2 + $(this).attr('class') + ' ' + $(this).attr('title');
    });
    return slist2.toLowerCase();
  }
};
...
var productReviews = function(){
  if ($('div#productpage').is('*')) {
    var numReviews = parseInt($('.reviews a[href="#reviews"]').text());

    if (numReviews > 50) {
      numReviews = "50+ reviews";
    } else if (numReviews > 30) {
      numReviews = "31-50 reviews";
    } else if (numReviews > 20) {
      numReviews = "21-30 reviews";
    } else if (numReviews > 15) {
      numReviews = "16-20 reviews";
    } else if (numReviews > 10) {
      numReviews = "11-15 reviews";
    } else if (numReviews > 7) {
      numReviews = "8-10 reviews";
    } else if (numReviews > 4) {
      numReviews = "5-7 reviews";
    } else if (numReviews > 1) {
      numReviews = "2-4 reviews";
    ...
    var $avgRating = $('div.reviews div.rating');

    if ($avgRating.hasClass('rating00')) {
      var avgRating = 0;
    } else if ($avgRating.hasClass('rating05')) {
      var avgRating = 0.5;
    } else if ($avgRating.hasClass('rating10')) {
      var avgRating = 1.0;
    } else if ($avgRating.hasClass('rating15')) {
      var avgRating = 1.5;
    } else if ($avgRating.hasClass('rating20')) {
      var avgRating = 2.0;
    } else if ($avgRating.hasClass('rating25')) {
      var avgRating = 2.5;
    } else if ($avgRating.hasClass('rating30')) {
      var avgRating = 3.0;
    } else if ($avgRating.hasClass('rating35')) {
      var avgRating = 3.5;
    } else if ($avgRating.hasClass('rating40')) {
      var avgRating = 4.0;
    } else if ($avgRating.hasClass('rating45')) {
      var avgRating = 4.5;
    } else if ($avgRating.hasClass('rating50')) {
      var avgRating = 5.0;
    }
    ...
    $('.reviews a[href="#reviews"], #tabs a[href="#customer-reviews"]').click(function(){
      s.linkTrackVars = 'events,products,eVar19,eVar20';
      s.linkTrackEvents = 'event21';
      s.events = 'prodView,event21';
      s.products = s.products + ';;;;eVar19=' + avgRating + '|eVar20=' + numReviews;
...
var emailSubscriptions = function(){
  if ($('div#mailinglist > h2.message').is('*')) {
    s.eVar24 = 'Subscribed';
...
/* lids homepage link_id tracker */
var hp_banner = function() {
    if (s.pageName == 'lids homepage') {

        /* Brand Scroller */
        $('#brandscroller .brands img').click(function() {
            var brands = $(this).attr('title');
            var brand = 'brandscroller_' + brands.replace(/[\']/g,'');
            _satellite.setCookie("hp_ic", _satellite.cleanText(brand));
        });

        /* Find Your Team */
        $('select', '#findteam').live('change', function(e) {
            var href = $('li a', '#findteam').attr('href');
            var sports = $('li a.on', '#findteam').attr('href').replace(/\//, '');
            var team = $(this).val();
            var n = team.lastIndexOf('/');
            team = team.substring(n + 1);
            var sp_tm = 'findteam_' + sports + '_' + team;
             _satellite.setCookie("hp_ic", _satellite.cleanText(sp_tm));
        });
...
s.linkInternalFilters="javascript:,lids.com,lids.ca";
...
/* Social Media config */
var smv = [
  {'name': 'Twitter'   , 'path': 'twitter.com/lids4hats'},
  {'name': 'Facebook'  , 'path': 'facebook.com/pages/Lids/6929942105'},
  {'name': 'Pinterest' , 'path': 'pinterest.com/lids4hats/'},
  {'name': 'Google+'   , 'path': 'plus.google.com/116762272824665936171'},
  {'name': 'YouTube'   , 'path': 'youtube.com/user/hatworldlids'}
];
...
function s_doPlugins(s) {
  ...
  if(s.pageName != 'lids homepage'){
    ...
    s.eVar3 = _satellite.readCookie('hp_ic');
    if (s.eVar3) {
      s.linkTrackEvents = 'event13';
      s.events = 'event13';
    }
    _satellite.removeCookie('hp_ic');
  }
  
  /* Visitor Type */
  var s_gvs = s.getVisitStart('s_gvs');
  if( s_gvs == 1 || s_gvs == '1' ) s.eVar13 = 'guest';
  
  
  /* External Campaign Tracking Code Extraction based on 'cpid' parameter */
  if( !s.campaign && s.getQueryParam( 'cpid' ) ) {
    s.campaign = s.getQueryParam( 'cpid' );
    if( s.campaign ) s.events = s.apl( s.events, 'event12', ',', 2 );
  }

  /* Internal Campaign Code Extraction based on 'link_id' parameter */
  
  if( !s.eVar3 && s.getQueryParam( 'link_id' ) ) {
    s.eVar3 = s.getQueryParam( 'link_id' );
    if( s.eVar3 ) s.events = s.apl( s.events, 'event13', ',', 2 );
  }


  /* Internal Recommendations Code Extraction based on 'src_id' parameter */
  if( !s.eVar15 && s.getQueryParam( 'src_id' ) ) {
    s.eVar15 = s.getQueryParam( 'src_id' );
    if( s.eVar15 ) s.events = s.apl( s.events, 'event22', ',', 2 );
  }

  /* Internal Search Terms */
  if( s.prop5 && s.getQueryParam( 'q' ) ) {
    s.prop5 = s.prop5.toLowerCase();
    var st = s.getQueryParam( 'q' ).toLowerCase();
    if( s.prop5 == 'no results' || s.prop5 == 'zero' ) {
      s.prop7 = s.getValOnce( st, 's_p7', 0);
      if( s.prop7 ) s.events = s.apl(s.events, 'event2', ', ',2);   
    } else {
      s.prop4 = s.getValOnce( st, 's_p4', 0);
      if( s.prop4 ) {
  ...
  /* Error Page */
  s.prevPage = s.getPreviousValue(s.pageName, 's_prevPage');
  if(s.pageName == '404 error page'){
    s.prop8 = location.href;
    s.pageName = s.pageName.toLowerCase();
    s.prop9 = s.prevPage.toLowerCase();
    s.pageType = 'errorPage';
  }

  /* Free Shipping */
  if( s.eVar6 ) if( s.eVar6.toLowerCase().indexOf('free')>-1) s.events = s.apl(s.events, 'event9', ',', 2);

  // if( !s.products ) s.products = ';';

  /* Custom ProdView Event */
  if( s.events ) {
    if( s.events.toLowerCase().indexOf('prodview')>-1 ) s.events = s.apl( s.events, 'event3', ',', 2 );
  }

  /* OrderID eVar and dedupe if necessary */
  if( s.purchaseID ) s.eVar7 = s.purchaseID;

  /* socialPlatforms - used for SocialAnalytics */
  s.socialPlatforms( 'eVar11' );

  /* Social Media Button Tracking */
  var smb = s.smb( smv, s.exitLinkHandler().toLowerCase() );
  if( smb ) {
    s.eVar18 = smb;
    s.events = s.apl( s.linkTrackEvents, 'event20', ',' ,2 );
    s.linkTrackEvents = 'event20';
    s.linkTrackVars = 'events,eVar18';
  }

  /* Find Method */
  if( !s.eVar1 ) s.eVar1 = s.pfm();

  /* Silverpop Message ID*/
  //if (!s.eVar16)
    //s.eVar16 = s.getQueryParam('SP_MID'); //Set eVar16 with Silverpop Mailing ID
  /* Silverpop Recipient ID */
  //if (!s.eVar17)
    //s.eVar17 = s.getQueryParam('SP_RID'); //Set eVar17 with Silverpop Recipient ID

  /* ExactTarget Message ID */
  if(!s.eVar16)
    s.eVar16 = s.getQueryParam('ET_CID');
  /* ExactTarget Recipient ID */
  if (!s.eVar17)
    s.eVar17 = s.getQueryParam('ET_RID');
  ...
  /* Web Property ID */
  s.eVar62 = window.location.hostname;
}
...
/*
 * Custom Plugin: Product Find Method - 
 */
s.pfm = function(){
  var s = this;
  var rtn = '', lif, dor, qp = s.getQueryParam('src_id').toLowerCase();
  if( s.campaign ) rtn = 'external campaign';
  else if( s.eVar2 ) rtn = 'search';
  else if( s.eVar3 ) rtn = 'internal campaign';
  else if( s.eVar15 ) rtn = 'recommendations campaign';
  else if( s.eVar4 ) {
    rtn = 'browse';
    s.eVar4 = s.eVar4.toLowerCase();
  } else if( qp == 'recs-orderConf' || qp == 'recs-shipConf' || qp == 'recs-abanCart' || qp == 'recs-prodView' || qp == 'recs-cartView' || qp == 'recs-zeroSearch') rtn = 'recommendations';
  else if( document.referrer ) {
    lif = s.linkInternalFilters.toLowerCase();
    dor = document.referrer.toLowerCase();
    if( lif.indexOf( dor ) > -1 ) rtn = 'external non-campaign';
  }
  return rtn;
}

/*
 *  Custom Plugin: Social Media Buttons.
 */ 
s.smb = function(sv,el){
  if( !sv || !el ) return '';
  for( var i=0; i<sv.length; i++ ){
    if( el.indexOf( sv[i].path.toLowerCase() )>-1 ) return sv[i].name;
  }
  return '';
}
...
s.socialAuthors=new Function("",""
                             ...
                             +"'SocialAuthor');s.Integrate.SocialAuthor.tEvar='eVar41';s.Integrate"
...
s.visitorNamespace="hatworld";
s.dc=122;
s.debugTracking=false;

/* Uncomment the following if 1st Party SSL Certificate has been installed */
if (window.location.hostname.indexOf('lids.ca') >= 0) {
  s.trackingServer="shop.lids.ca";
  s.trackingServerSecure="shops.lids.ca";
} else {
  s.trackingServer="shop.lids.com";
  s.trackingServerSecure="shops.lids.com";
}
...
s.linkTrackVars = s.apl(s.linkTrackVars,'contextData',',',2);
s.loadModule("Media");
s.Media.trackMilestones = "5,10,25,50,75";
s.Media.trackWhilePlaying = true;
s.Media.trackUsingContextData = true;
s.Media.trackVars = 'contextData';

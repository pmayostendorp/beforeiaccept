...
new SC_StubQP('cbsihelpcentersite_s');
...
var om = {
  ...
  data: {
    //config data
    is_dev: false,
    internal_filters:"cbsi.com",
    rsids:"cbsihelpcentersite",
    rsids_dev:"cbsihelpcentersite-dev",
    sid:"cs",
    site_type:"desktop web",
  ...
  ss:'',  //internal s object

  
  //------------------- public functions ----------------------//

  // public alias of om.trackView()
  t: function(p) {
    var self=this;
	//console.log(p);
  ...
  trackSocialClick: function(p) {
    var self=this;
    var sMap = {
      linkTrackVars:"events,eVar2,eVar40",
      linkTrackEvents:"event10",
      events:"event10",
      eVar40: p.id
    }
    self.trackEvent(sMap,true,'o','trackSocialClick');
  },
  // track generic module clicks
  trackClick: function(p,obj,href) {
    var self=this;
    var sMap = {
      linkTrackVars:"events,channel,eVar5,eVar46",
      linkTrackEvents:"event19",
      events:"event19",
      eVar46: p.item
    }
    self.trackEvent(sMap,obj,'o','trackClick',href);
  },
  // track search button
  trackSearchSubmit: function(p) {
    var self=this;
    var sMap = {
      linkTrackVars:"events,channel,eVar10,eVar41",
      linkTrackEvents:"event48",
      events:"event48",
      eVar41:p.term
    }
    self.trackEvent(sMap,true,'o','trackSearchSubmit','');
  },
  // track next page button
  trackNextPage: function(pn) {
    var self=this;
	omdata.context.page_num=pn;
    self.trackView(omdata);
  },
  // track submit question button
  trackSubmitQuestion: function() {
    var self=this;
    var sMap = {
      linkTrackVars:"events,channel,eVar10",
      linkTrackEvents:"event2",
      events:"event2"
    }
    self.trackEvent(sMap,true,'o','trackSubmitQuestion','');
  },
  // track submit feedback button
  trackSubmitFeedback: function() {
    var self=this;
    var sMap = {
      linkTrackVars:"events,channel,eVar10",
    ...
    self.trackEvent(sMap,true,'o','trackSubmitFeedback','');
  },
  ...
  mapSvars: function(p) {
    ...
    if(c.forum_thread){
      self.setEvar(s,'67',c.forum_thread.toLowerCase());
	  self.setProp(s,'67',"D=v67");
    }
	if(c.evt_forum_view){d.events.push('event26');}
	
	
	/*------------------- common variable mappings ----------------------*/
    if(d.author_ids){
      self.setList(s,'3',d.author_ids);
    }
    if(d.author_names){
      self.setEvar(s,'37',d.author_names);
      self.setProp(s,'37',"D=v37");
    }
    if(d.blog_id){
      self.setEvar(s,'26',d.blog_id);
      self.setProp(s,'26',"D=v26");
    }
    if(d.brand){
      self.setEvar(s,'1',d.brand.toLowerCase());
      self.setProp(s,'1',"D=v1");
    }
    if(d.channel){
      self.setChannel(s,d.channel.toLowerCase());
    }
    if(d.content_id){
      self.setEvar(s,'30',d.content_id);
      self.setProp(s,'30',"D=v30");
    }
    if(d.content_title){
      self.setEvar(s,'20',d.content_title.toLowerCase());
      self.setProp(s,'20',"D=v20");
    }
    if(d.content_topic){
      self.setEvar(s,'23',d.content_topic.toLowerCase());
      self.setProp(s,'23',"D=v23");
    }
    if(d.content_type){
      self.setEvar(s,'22',d.content_type.toLowerCase());
      self.setProp(s,'22',"D=v22");
    }
    if(d.edition){
      self.setEvar(s,'2',d.edition.toLowerCase());
      self.setProp(s,'2',"D=v2");
    }
    if(d.ontology_id){
      self.setEvar(s,'13',d.ontology_id);
      self.setProp(s,'13',"D=v13");
    }
    if(d.page_guid){
      self.setEvar(s,'24',d.page_guid);
      self.setProp(s,'24',"D=v24");
    }
    if(d.page_name){
      self.setPageName(s,d.page_name);
    }else{
      self.setPageName(s,d.sid+':'+document.location.pathname+document.location.search);
    }
	if(d.page_num){
      self.setPageName(s,d.sid+':'+document.location.pathname+document.location.search+':'+d.page_num);
    }
    if(d.page_type_id){
      self.setEvar(s,'12',d.page_type_id);
      self.setProp(s,'12',"D=v12");
    }
    if(d.page_type){
      self.setEvar(s,'10',d.page_type.toLowerCase());
      self.setProp(s,'10',"D=v10");
    }
    if(d.search_dimension){
      self.setEvar(s,'42',d.search_dimension.toLowerCase());
      self.setProp(s,'42',"D=v42");
    }
    if(d.search_term){
      self.setEvar(s,'41',d.search_term.toLowerCase());
      self.setProp(s,'41',"D=v41");
    }
    if(d.section){
      self.setChannel(s,d.section.toLowerCase());
    }
    if(d.page_title){
      self.setEvar(s,'8',d.page_title.toLowerCase());
      self.setProp(s,'8',"D=v8");
    }
    if(d.site_type){
      self.setEvar(s,'3',d.site_type.toLowerCase());
      self.setProp(s,'3',"D=v3");
    }
    if(d.err){
      self.setErrorPage(s);
      self.setChannel(s,'error');
      self.setPageName(s,d.sid+':'+d.err+':error');
    }
    if(d.url){
      self.setEvar(s,'7',d.url);
      self.setProp(s,'7',"D=v7");
    }
	if(d.user_agent){
      self.setProp(s,'9',d.user_agent);
    }
    ...
    if(rsids) {
      var om_sacc=rsids.split(',');
      if (om_sacc[0]!=='undefined') {
        self.setEvar(s,'5',om_sacc[0]);
        self.setProp(s,'5',"D=v5");
      }
    }
    
	if(d.evt_content_view){d.events.push('event30');}
	if(d.evt_gallery_view){d.events.push('event32');}
	if (d.events.length) {
      self.setEvents(s,d.events.join(','));
    }
  },
  ...
  initScode: function() {
    ...
    trackingSettings = {
      ...
      linkInternalFilters: 'javascript:,' + self.data.internal_filters + ',doubleclick.net,cbsi.com',
    ...
    accountSettings = {
      visitorNamespace: "cbsinteractive",
      trackingServer: "om.cbsi.com",
      trackingServerSecure: "som.cbsi.com"
    };

    if (!!self.data.is_dev) {
      s = s_gi(self.data.rsids_dev);
    }else{
      s = s_gi(self.data.rsids);
    }
    self.extend(s,trackingSettings,accountSettings);
    self.initPlugins(s);
    self.ss=s;
    window.s=s; //create a globla version to support TagManager function
  },
  ...
  trackEvent: function(v,obj,lt,ln,href) {
    var self = this;
	var s = self.ss;
    self.extend(s,v); //insert variable mapping into s obj
    if (typeof(obj)=='object' && href=='navigate') {
      s.tl(obj,lt,ln,null,href);
    } else if (typeof(obj)=='boolean' && href.match('/.+\.\w\w.*/')) {
      //match a valid href string
      s.tl(obj,lt,ln,null,function(){window.location=href})
    } else {
      s.tl(obj,lt,ln);
    }
    self.clearEvent(v); //reset all vars
  },
  // clear any Custom Events settings
  clearEvent: function(v) {
    var self = this;
	var s = self.ss;
    var ce = {};
    for(key in v) {
      if (key=="linkTrackVars"||key=="linkTrackEvents"){
        ce[key]="None";
      }else{
        ce[key]="";
      }
  ...
  setPageName: function(s, value) {
    if (!!value) {
      s['pageName'] = value;
    }
  },
  // set eVar variables
  setEvar: function (s, id, value) {
    if (!!value) {
      s['eVar' + id] = value;
    }
  },
  // set prop variables
  setProp: function (s, id, value) {
    if (!!value) {
      s['prop' + id] = value;
    }
  },
  // set the events variable
  setEvents: function (s, value) {
    if (!!value) {
      s['events'] = value;
    }
  },
  // set the list variables
  setList: function (s, id, value) {
    if (!!value) {
      s['list' + id] = value;
    }
  },
  // set the channel variable
  setChannel: function(s, value) {
    if (!!value) {
      s['channel'] = value;
    }
  },
  // set error pagetype
  setErrorPage: function(s) {
    s['pageType'] = 'errorPage';
  },
  // check valid object key
  isValidObjectKey: function (key, object) {
    return object.hasOwnProperty(key);
  },
  
  // To easily extend objects or make copies of objects.
  extend: function () {
    var args = arguments,
    target = args[0] || {},
    length = args.length,
    ...
    /**
     * Checks keys in extension for defined values and assigns them to
     * target using the same keys respectively.  If a key didn't exist in
     * target before but existed in extension, the key will be added to
     * target along with its corresponding value from extension.
     */
    for (i = 1; i < length; i++) {
      extension = args[i];
      for (key in extension) {
        if (true === this.isValidObjectKey(key, extension)) {
          value = extension[key];
          if (undefined !== value) {
            target[key] = value;
          }
  ...
  // Init Plugins
  initPlugins: function(s) {
    var om = this;
    s.usePlugins=true;
	s.doPlugins= function(s) {
      /* Add calls to plugins here */
      var om_camp = [];
	  if (s.getQueryParam('ftag')!=='') {
        om_camp.push('ttag:' + s.getQueryParam('ftag'));
      }
      if (s.getQueryParam('ttag')!=='') {
        om_camp.push('ttag:' + s.getQueryParam('ttag'));
      }
	  if (s.getQueryParam('s_cid')!=='') {
        om_camp.push('ttag:' + s.getQueryParam('s_cid'));
      }
      if (s.getQueryParam('utm_source')!=='') {
        om_camp.push('utm_source:' + s.getQueryParam('utm_source'));
      }
      if (s.getQueryParam('utm_medium')!=='') {
        om_camp.push('utm_medium:' + s.getQueryParam('utm_medium'));
      }
      if (s.getQueryParam('utm_campaign')!=='') {
        om_camp.push('utm_campaign:' + s.getQueryParam('utm_campaign'));
      }
	  if (s.getQueryParam('vndid')!=='') {
        om_camp.push('vndid:' + s.getQueryParam('vndid'));
      }
      s.campaign = om_camp.join('|');
      s.eVar4 = s.getQueryParam('tag');
      s.eVar45 = s.getQueryParam('part');
      s.prop45 = s.getQueryParam('part');
      s.eVar53 = s.getNewRepeat(30, 's_getNewRepeat');
      s.eVar54 = s.getDaysSinceLastVisit('s_lv_'+om.getSiteId());
...
/********************** END PLUGINS SECTION *************************/
  ...
  initIntegrate: function(s) {
    ...
    s.Integrate.ClickTale.setVars=function(s,p){
        if(typeof s.readCookie=='function'){
          s[p.sessionVar]=s.readCookie("WRUID")
        }
      }
...
s.setTagContainer("cbsihelpcentersite_s");

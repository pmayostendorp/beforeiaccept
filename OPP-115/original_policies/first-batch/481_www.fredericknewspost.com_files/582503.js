(function(){
   ...
   */
  var toutInjector = (function(){
    /*
     * Start of Tout Injector module
     */
    var ToutInjector = function(config){
      // set some defaults
      config = config || {};
      config.root = config.root || window;
      config.$ = config.$ || jQuery;
      config.el = config.el || ".tout-mid-article-iframe";

      this.$ = config.$;
      this.root = this.$(config.root);
      this.targetFrame = this.$(config.el);
      this.targetFrameWindow = this.targetFrame[0].contentWindow;
      this.targeting_delay = 100;
      this.allowed_offset= this.targetFrame.height() / 2;
      this.playing = false;
    };

    ToutInjector.prototype.targetIsInRange = function (){
      var $elem = this.targetFrame;
      var allowedOffset = this.allowed_offset;
      var viewportTop = this.root.scrollTop();
      var viewportBottom = viewportTop + this.root.height();
      var elemTop = $elem.offset().top;
      var elemBottom = elemTop + $elem.height();
      var bottomBoundary = viewportBottom + allowedOffset;
      var topBoundary = viewportTop - allowedOffset;
      var inRange = ((elemBottom <= bottomBoundary) && (elemTop >= topBoundary));

      return inRange;
    };

    ToutInjector.prototype.engage = function (){
      if(this.lockedOn){return}
      this.lockedOn = true;

      var self = this;
      this.engage_delay = setTimeout(function(){
        if(self.isPlaying()){return}
        self.playOrPause();
      }, this.targeting_delay);

      window.clearTimeout(this.disengage_delay);
    };

    ToutInjector.prototype.disengage = function (){
      if(!this.lockedOn){return}
      this.lockedOn = false;

      var self = this;
      this.disengage_delay = setTimeout(function(){
        if(self.isntPlaying()){return}
        self.playOrPause();
      }, this.targeting_delay);

      window.clearTimeout(this.engage_delay);
    };

    ToutInjector.prototype.playOrPause = function(){
    ...
    ToutInjector.prototype.play = function(){
      this.playing = true;
      this.targetFrameWindow.postMessage("play", '*');
    };

    ToutInjector.prototype.pause = function(){
      this.playing = false;
      this.targetFrameWindow.postMessage("pause", '*');
    };

    ToutInjector.prototype.stop = function(){
      this.playing = false;
      this.targetFrameWindow.postMessage("stop", '*');
    };

    ToutInjector.prototype.isPlaying = function(){
      return this.playing;
    };

    ToutInjector.prototype.isntPlaying = function(){
      return !this.playing;
    };

    ToutInjector.prototype.onScroll = function(e){
      if(this.targetIsInRange()){
        if(this.lockedOn){return}
        this.engage();
      }else{
  ...
  var noConflictSetup = function(modName){
    var previous = window[modName];
    delete window[modName];
    return previous;
  };

  var noConflictRun = function(modName, previous){
    var current = window[modName];
    delete window[modName];
    if(previous){
      window[modName] = previous;
    }
    return current;
  };


  var theirRSVP = noConflictSetup("RSVP");
  ...
  var rsvp = noConflictRun("RSVP", theirRSVP);
  ...
  TOUT.propertyUid = "582503";

  /*
    TOUT.register - register a module on the "TOUT" global
    Usage:
      with a namespace:    TOUT.register("foo", {bar: myFunc}) => TOUT.foo.bar = myFunc
      without a namespace: TOUT.register({bar: myFunc}) => TOUT.bar = myFunc
   */
  var register = TOUT.register = function(){
    var root = window.TOUT,
      mod = arguments[0];
  ...
  register("utils", {
    ...
    getFirstRegexMatch: function(regex, str){
    ...
    isFeatureEnabled: function(featureName){
      return this.window().location.hash.indexOf(featureName) !== -1;
    }
  });

  register({
    _scriptIsReady: function(){
      // create a promise which immediately resolves
      var ready = new TOUT.utils.rsvp.Promise(function(resolve, reject) {
        resolve();
      });

      // replace the "placeholder" onReady function defined by the embed code
      //  with the actual implementation
      TOUT.onReady = function(func){
        ready.then(func);
      };

      // if the "placeholder" onReady function was called, loop through all of the functions
      //  passed to it, and send them to the new onReady function
      if(TOUT.onReadyCallbacks){
        for(var i=0; i < TOUT.onReadyCallbacks.length; i++){
          TOUT.onReady(TOUT.onReadyCallbacks[i]);
        }
      }
    }
  });
  ...
  register({
    ...
    _attemptAutoStart: function(){
      if(this.getBrandUid() !== "e91982"){
        this.start();
      }
    },
    ...
    start: function(props){
      ...
      // brand uid is the only required attribute.  if it's not set, then bail
      if(!this.getBrandUid()){ return false; }

      this._fetchProduct("sdk_loaded");

      if(this.config.get("autoFetch")){
        this.fetch();
      }
    ...
    getBrandUid: function(){
      return this._getBrandUidFromHost() || this._getBrandUidFromConfig() || this._getBrandUidFromSdkElement();
    },
    getPageMetadata: function(){
      var _document = this.utils.document();

      return {
        canonicalUrl: this.getCanonicalUrl(),
        externalArticleId: this.getExternalArticleId(),
        pageHost: _document.location.host,
        pageUrl: _document.location.href,
        pageReferrer: _document.referrer,
        products: {
          mid_article: {
            feed_uid: this._getAttributeForSelector('.tout-mid-article', 'data-tout-feed-uid')
          }
        }
      };
    },
    getCanonicalUrl: function(){
      var canonicalUrl = this.utils.getCanonicalLinkHref() || this.utils.getMetaTagContentByProperty("og:url");

      if(canonicalUrl && canonicalUrl[0] === "/"){
        canonicalUrl = this.utils.getWindowLocationOrigin() + canonicalUrl;
      }
    ...
    getExternalArticleId: function(){
      var brandUid = this.getBrandUid();
      return this._getExternalArticleIdFromDefaultMetaTag() || this._getExternalArticleIdFromBrandConfiguration(brandUid);
    },
    _getBrandUidFromConfig: function(){
      return this.propertyUid || this.property || this.brandUid || this.branduid;
    },
    _getAttributeForSelector: function(selector, attribute){
      var el = document.querySelectorAll(selector)[0];
      return el ? el.getAttribute(attribute) : null;
    },
    _getBrandUidFromSdkElement: function(){
      var sdkElement = document.getElementById("tout-js-sdk");
      if(sdkElement){
        var attr = sdkElement.getAttribute("data-property-uid");
        if(attr){ return attr; }
      }
    },
    _getBrandUidFromHost: function(){
      // TODO: can this polyfill be removed now?
      var host = this.utils.document().location.host;

      if(host.indexOf("normantranscript") > -1){
        return "unp4bw";
      } else if(host.indexOf("goerie") > -1){
        return "na5hfj";
      } else if(host.indexOf("insidebayarea") > -1){
        return "7yghg2";
      } else if(host.indexOf("rrstar") > -1){
        return "tjge5p";
      } else if(host.indexOf("pressrepublican") > -1){
        return "ec4548";
      }
    },
    _getExternalArticleIdFromDefaultMetaTag: function(){
      return this.utils.getMetaTagContentByProperty("tout:article:id");
    },
    _getExternalArticleIdFromBrandConfiguration: function(brandUid){
      var _window = this.utils.window();

        // TODO: it would be nice if this was like `brand.hasUrlRegexMetatag()` (ie an object for the brand)
        if(brandUid === 'tjge5p' || brandUid === '91fb4b' || brandUid === '6b4432' || brandUid === 'd89c6f' ||
           brandUid === '753cb7' || brandUid === 'b29eec' || brandUid === 'ad4adf' || brandUid === '67b22b' ||
           brandUid === 'dbac22' || brandUid === '85d9bd' || brandUid === '4a85cc' || brandUid === 'b7a25b' ||
           brandUid === '010393' || brandUid === 'cefa6f' || brandUid === 'e896af' || brandUid === 'e7b462' ||
           brandUid === '94a144' || brandUid === 'aee3d9' || brandUid === '2a530d' || brandUid === '656e86' ||
           brandUid === 'f40676'){
          var unique_id = this.utils.getMetaTagContentByProperty("vf:unique_id");
          if (unique_id) {
            return unique_id.toLowerCase();
          }
        } else if(brandUid === "7yghg2"){
          var match = this.utils.getFirstRegexMatch(/ci_\w+/, _window.location.pathname);
          // TODO: figure out how the "remove ci_" can be configured in annotation config..
          if(match){
            return match.replace("ci_", "");
          }
        } else if(brandUid === "4fb5e6" || brandUid === "1f8d5d" || brandUid === "24fd91" || brandUid === "000e33"){
          return this.utils.getFirstRegexMatch(/article_[A-Za-z0-9\-\_]+/, _window.location.pathname);
        } else if(brandUid === "5eabc5"){
          return this.utils.getFirstRegexMatch(/\d{9}/, _window.location.pathname);
        }

      return undefined;
    },

    _getSdkHost: function(){
      return TOUT.sdkHost || this.config.get("sdkHost");
    }
  });

  register({
    fetch: function(productName){
      this._beforeFetch(this.getBrandUid());

      if(productName){
        return this._fetchProduct(productName);
      } else {
        // TODO: return a hash of promises (not needed now, but will be helpful later)
        this._fetchProduct("right_rail");
        this._fetchProduct("top_article");
        this._fetchProduct("mid_article", this.getPageMetadata()['products']['mid_article']);
        this._fetchProduct("bottom_article");
      }
    },
    _fetchProduct: function(productName, queryParamsToMerge){
      var origin = "http://" + this._getSdkHost(),
        path = {
          top_article: "/embeds.js",
          right_rail: "/autocube.js",
          mid_article: "/mid_article_player.js",
          bottom_article: "/below_article_ticker.js",
          sdk_loaded: "/tracker.js"
        }[productName],
        pageMetadata = this.getPageMetadata();

      var queryParams = {
        'brand_uid': encodeURIComponent(this.getBrandUid()),
        'sdk_version': encodeURIComponent(TOUT.version),
        'parent_url': encodeURIComponent(pageMetadata.pageUrl),
        'content_referrer': encodeURIComponent(pageMetadata.pageReferrer)
      };

      if(pageMetadata.canonicalUrl){
        queryParams["og_url"] = encodeURIComponent(pageMetadata.canonicalUrl);
      }
      if(pageMetadata.externalArticleId){
        queryParams["external_article_id"] = encodeURIComponent(pageMetadata.externalArticleId);
      }
      var query = "?";
      for (var item in queryParamsToMerge) {
        if(queryParamsToMerge[item]) {
          queryParams[item] = queryParamsToMerge[item];
        }
      }
      ...
      // TODO: we can actually cache this: if the url is the same, we can return an existing promise.
      var that = this;
      return new this.utils.rsvp.Promise(function(resolve, reject){
        // TODO: these should be converted into ajax json calls, then we can drop the global _onFetchTopArticleSuccess handlers
        that._insertScriptTag(origin + path + query);

        if(productName === "top_article"){
          that.config.set("_onFetchTopArticleSuccess", function(data){
            resolve(data);
          });

          that.config.set("_onFetchTopArticleError", function(data){
            reject(Error(data.error));
          });
        }
      });
    },
    _beforeFetch: function(brandUid){
      // TODO: can this be removed now?
      if(brandUid === "7yghg2"){
        document.getElementById("toutVideoDiv").style.display = "none";
      }
    },
    _insertScriptTag: function(url){
  ...
  (function(){
    ...
    register("config", {
      ...
      set: function(){
        // the default format expected is 1 argument: set({key: value})
        var data = arguments[0];

        // also support a format where they pass 2 arguments: set(key, value)
        if(arguments.length === 2){
          data = {};
  ...
  //We will want an eventbus later, but not right now.  To bring this back:
  //  1) Find the specs with "xdescribe"
  //  2) Include eventEmitter2 in sdk.js
  //register({
  //  events: new eventEmitter.EventEmitter2({ wildcard: true, maxListeners: 0 })
  //});

  TOUT._scriptIsReady();
  TOUT._attemptAutoStart();
})();

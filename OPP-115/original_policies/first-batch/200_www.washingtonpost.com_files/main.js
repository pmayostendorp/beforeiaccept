...
},{"./lib/analytics-services/chartbeat":2,"./lib/analytics-services/comscore":4,"./lib/analytics-services/effective-measure":6,"./lib/analytics-services/omniture":11,"./lib/element-tracker":14,"./lib/init-report":15,"./lib/track-scrolling":17}],2:[function(require,module,exports){
...
module.exports = function initChartbeat(config) {
  ...
  jQuery(document).ready(function() {
    var _chartbeatUrl = (("https:" == document.location.protocol) ? "https://" : "http://") + "static.chartbeat.com/js/chartbeat.js";
...
},{"./init":5,"dup":2}],5:[function(require,module,exports){
...
arguments[4][2][0].apply(exports,arguments)
},{"./init":7,"dup":2}],7:[function(require,module,exports){
...
var endpoint = 'https://loxodo-analytics.ext.nile.works/api/save';
...
function getUUID() {
  // TODO: set cookie if no localStorage
  var uuid;
  if (_utils.hasStorage) {
    if (localStorage.getItem('wp_vi')) {
      uuid = localStorage.getItem('wp_vi');
    } else {
      uuid = generateUUID();
      localStorage.setItem('wp_vi', uuid);
    }
  } else {
    uuid = generateUUID();
  }
...
function sendData(data, options) {
  ...
  return jQuery.ajax(ajaxOptions)
    .then(function(response) {
      if (_utils.shouldLog) console.log('LOX:', response);
    })

    .then(null, function(error) {
      if (_utils.shouldLog) console.error('LOX:', error);
    });
}

},{"../../../util/utils":18}],9:[function(require,module,exports){
...
function loadOmniture(config) {
  ...
  */
  ...
  s_account = window.s_account || "wpniwashpostcom";
  s = s_gi(s_account);
  ...
  function s_doPlugins(s) {
    ...
    wp_defaultEvents = s.isNotIframe() ? 'event1' : 'event32';
    ...
    */
    ...
    if (typeof s.prop71 == 'undefined'){
      s.prop71 = s.getQueryParam('tid');
      s.eVar71 = s.prop71;
    }
  ...
  */
  ...
  s.initForTwp = function() {
    ...
    */
    ...
    if (typeof(wp_impression_tracking) != "undefined") {
      s.prop61 = wp_impression_tracking;
    }
    ...
    s.prop72 = ( !! window.localStorage && !! localStorage.opbandit && localStorage.opbandit == 'F') ? 'eligible' : 'ineligible';
    if (!window.wp_pvid) {
      wp_pvid = Math.round(Math.random() * Math.pow(10, 13)) + '-' + new Date().getTime();
    }
    s.prop73 = wp_pvid;
  ...
  s.nullifyOnResend = ['events', 'campaign', 'prop20', 'prop27', 'prop71', 'eVar3', 'eVar13', 'eVar20', 'eVar29', 'eVar71'];
  ...
  s.sendPageViewToComscoreDontPanic = function() {
    if (_utils.shouldLog) console.log("DON'T PANIC: Comscore will be called on the live site.");
  };
  ...
  function waitForPaywallTracking(setTimer, checkCount) {
    checkCount += 1;
    if (_utils.shouldLog) {
      console.log("OMNITURE - WAIT FOR PAYWALL: " + setTimer);
      console.log("OMNITURE - CHECKCOUNT: " + checkCount);
    }
    if ((s && window.TWP && TWP.Identity && TWP.Identity.paywall && TWP.Identity.paywall.pwresp) || checkCount > 6 || setTimer != true) {
      ...
      if (_utils.shouldLog) {
        console.log("OMNITURE - PAYWALL VARIABLES: 60: " + s.eVar60 + " 64:" + s.eVar64 + " 65: " + s.eVar65 + " 66: " + s.eVar66);
      }
    } else {
      setTimeout(function() {
    ...
    if (window.TWP &&
        ...
        !TWP.Identity.paywall.pwresp) {
          ...
          if (window.location && location.pathname && /^\/sf\//.test(location.pathname)) {
            jQuery(document).ready(function() {
              setTimeout(function() {
                if (_utils.shouldLog) console.log('OMNITURE - timeout');
                waitForPaywallTracking(true, checkCount = 0);
              }, 500);  
            });
          }
...
},{"../../../util/utils":18,"../lox":9,"./state":13}],11:[function(require,module,exports){
var init  = require('./init');
...
},{"./init":12,"./state":13}],12:[function(require,module,exports){
...
},{"../../state-store":16}],14:[function(require,module,exports){
...
},{"../util/utils":18}],16:[function(require,module,exports){
var _utils = require('../util/utils');
...
StateStore.prototype.ready = function(cb) {
  if (this.readyState) {
    if (_utils.shouldLog) console.log(this.service + ' - ready');
    return cb();
  } else {
    if (_utils.shouldLog) console.log(this.service + ' - waiting for ' + this.eventName);
    var _self = this;
    jQuery(window.document).one(this.eventName, function() {
      _self.readyState = true;
      if (_utils.shouldLog) console.log(_self.service + ' - ready');
      return cb();
    });
  }
};
...
},{"../util/utils":18}],17:[function(require,module,exports){
module.exports = function(){
...
var shouldLog = hasConsole && (debugAnalytics && RegExp.$1 === 'log');

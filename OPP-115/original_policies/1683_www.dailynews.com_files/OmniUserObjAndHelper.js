...
function omniObj(){
	...
	function loadValues(){
		...
		for (var i=0; i<valueArray.length;i++){
			...
			try{
				this[elementArray[0]]=elementArray[1];
			}catch(err){
			}finally{
			}
		}
	...
	function saveValues(){
		...
		for (sProperty in this) {
        	if (sProperty && typeof(this[sProperty]) != "function"){
				cookieValue += sProperty + "=" + this[sProperty]+";";
			}
      	}
		setCookie(this.COOKIE_NAME, cookieValue, 365*10, '/', getBaseDomain(), null);
	}
	...
	function updateValues(){
		...
		if (getCookie('fPage')==''){
			this.fPage=true;
			setCookie('fPage','true', null, '/', getBaseDomain(), null);
		}else{
			this.fPage=false;
			setCookie('fPage','false', null, '/', getBaseDomain(), null);
		}
		...
		if (window.location.href.indexOf("forced=true") != -1 || this.initRegType == 'Premium'){

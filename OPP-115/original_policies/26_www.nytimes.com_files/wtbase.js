/*
$Id: wtbase.js 134040 2013-10-29 16:07:53Z richard.pinales $
 */
 var qryFlag = 0;
function dcsCQP(){
...
//	WT.sp="@@SPLITVALUE@@";
	...
	if(wt_contentGroup == "Email This"){
		...
		// setting DCS.dcssip value with the hostname
		var originalLink = '';
		if( $('articleBody') && ($('articleBody').down('p.sentTo a'))) { 
			var articleURLNode = $('articleBody').down('p.sentTo a');
			originalLink = articleURLNode.href;
		}
		else if( document.forms[1] && document.forms[1].elements['url']) {
			originalLink = document.forms[1].elements['url'].value; 
		}
		var reObj = new RegExp('^(?:f|ht)tp(?:s)?\://([^/]+)', 'im');
		originalLink = unescape(originalLink);
		DCS.dcssip = originalLink.match(reObj)[1].toString();	
	}
...
//adding a call to EventTracker to capture values sent to WebTrends
var originalDcsCreateImage = dcsCreateImage;
dcsCreateImage = function (P) {
    originalDcsCreateImage(P);
    eventTrackerPixel(P);
};

var eventTrackerPixel = function (P) {
    'use strict';
    var subject = ['dcsmultitrack', 'notitle'];
    if (typeof WT !== 'undefined' && WT.z_dcsm === '1' && typeof WT.ti === 'string' 
            && WT.ti !== '') {
        subject[1] = WT.ti.toLowerCase()
                    .replace(/[^\-0-9a-z]/gi, '-')
                    .replace(/[\-]{2,}/g, '-')
                    .substring(0, 50);
    }
    if (typeof NYTD.dcsMultiTrackEventTracker === 'undefined') {
        NYTD.dcsMultiTrackEventTracker = new NYTD.EventTracker();
    }
    NYTD.dcsMultiTrackEventTracker.pixelTrack({
